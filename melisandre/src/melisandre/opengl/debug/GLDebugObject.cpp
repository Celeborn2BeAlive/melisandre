...
#include <melisandre/opengl/utils/GLutils.hpp>
...
GLDebugObject::GLDebugObject() {
    ...
    VAO.vertexAttribOffset(VBO.glId(), 0, 3, GL_FLOAT, GL_FALSE,
                           sizeof(Vertex), MLS_OFFSETOF(Vertex, position));
    VAO.vertexAttribOffset(VBO.glId(), 1, 3, GL_FLOAT, GL_FALSE,
                           sizeof(Vertex), MLS_OFFSETOF(Vertex, normal));
}
...
void GLDebugObject::setInstanceBuffer(GLuint instanceBuffer) {
    ...
    VAO.vertexAttribOffset(instanceBuffer, 2, 3, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, color));
    glVertexAttribDivisor(2, 1);

    VAO.enableVertexAttrib(3);
    VAO.vertexAttribOffset(instanceBuffer, 3, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvpMatrix) + 0 * sizeof(Vec4f));
    glVertexAttribDivisor(3, 1);

    VAO.enableVertexAttrib(4);
    VAO.vertexAttribOffset(instanceBuffer, 4, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvpMatrix) + 1 * sizeof(Vec4f));
    glVertexAttribDivisor(4, 1);

    VAO.enableVertexAttrib(5);
    VAO.vertexAttribOffset(instanceBuffer, 5, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvpMatrix) + 2 * sizeof(Vec4f));
    glVertexAttribDivisor(5, 1);

    VAO.enableVertexAttrib(6);
    VAO.vertexAttribOffset(instanceBuffer, 6, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvpMatrix) + 3 * sizeof(Vec4f));
    glVertexAttribDivisor(6, 1);

    VAO.enableVertexAttrib(7);
    VAO.vertexAttribOffset(instanceBuffer, 7, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvMatrix) + 0 * sizeof(Vec4f));
    glVertexAttribDivisor(7, 1);

    VAO.enableVertexAttrib(8);
    VAO.vertexAttribOffset(instanceBuffer, 8, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvMatrix) + 1 * sizeof(Vec4f));
    glVertexAttribDivisor(8, 1);

    VAO.enableVertexAttrib(9);
    VAO.vertexAttribOffset(instanceBuffer, 9, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvMatrix) + 2 * sizeof(Vec4f));
    glVertexAttribDivisor(9, 1);

    VAO.enableVertexAttrib(10);
    VAO.vertexAttribOffset(instanceBuffer, 10, 4, GL_FLOAT, GL_FALSE,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, mvMatrix) + 3 * sizeof(Vec4f));
    glVertexAttribDivisor(10, 1);

    VAO.enableVertexAttrib(11);
    VAO.vertexAttribIOffset(instanceBuffer, 11, 4, GL_UNSIGNED_INT,
                           sizeof(GLDebugObjectInstance), MLS_OFFSETOF(GLDebugObjectInstance, objectID));
...
void GLDebugObject::draw() {
    ...
    glDrawElements(primType, GLsizei(IBO.size()), GL_UNSIGNED_SHORT, 0);
}

void GLDebugObject::draw(size_t nbInstances) {
    VAO.bind();
    IBO.bind(GL_ELEMENT_ARRAY_BUFFER);
    glDrawElementsInstanced(primType, GLsizei(IBO.size()), GL_UNSIGNED_SHORT, 0, GLsizei(nbInstances));
}
