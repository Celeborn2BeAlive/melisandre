...
#include <melisandre/system/DebugLog.hpp>
#include <melisandre/system/threads.hpp>

#pragma warning( push )
#pragma warning( disable : 4800 ) // type conversion warning from int to bool
#pragma warning( disable : 4996 ) // Unsafe strncopy
...
#pragma warning( push )
#pragma warning( disable : 4312 ) // type conversion warning issued by stb
...
//        }
    ...
    void storeImage(const std::string& filepath, const Image& image) {
        ...
        for (int j = image.getHeight() - 1; j >= 0; --j) {
            for (auto i = 0u; i < image.getWidth(); ++i) {
                writePtr[0] = unsigned char(min(255.f, ptr->r * 255.f));
                writePtr[1] = unsigned char(min(255.f, ptr->g * 255.f));
                writePtr[2] = unsigned char(min(255.f, ptr->b * 255.f));
                writePtr[3] = unsigned char(min(255.f, ptr->a * 255.f));
    ...
    struct EXRSetGlobalThreadCountRAII {
        EXRSetGlobalThreadCountRAII() {
            Imf::setGlobalThreadCount(getSystemThreadCount());
        }
    ...
    static Vec3f computeNMSE(const Image& reference, const Image& image) {
        ...
        for(auto i: range(size)) {
            ...
            if(!reduceLogicalOr(mls::isnan(diff)) && !reduceLogicalOr(mls::isinf(diff))) {
    ...
    float computeLocalVariance(const Image& image, Vec2u pixel, int windowSize) {
        auto localMean = 0.f;
        auto count = 0u;
        for(auto j = -windowSize; j <= windowSize; ++j) {
            for(auto i = -windowSize; i <= windowSize; ++i) {
                if(image.contains(int(pixel.x) + i, int(pixel.y) + j) && (i != 0 || j != 0)) {
                    auto value = image(int(pixel.x) + i, int(pixel.y) + j);
                    if(value.w) {
                        value /= value.w;
                    }
                    localMean += length(Vec3f(value));
                    ++count;
                }
            }
        }
        ...
        auto value = image(pixel.x, pixel.y);
        if(value.w) {
            value /= value.w;
        }
        auto x = length(Vec3f(value));
        return abs(x - localMean) / localMean;
    }

    float computeLocalMean(const float* pData, size_t w, size_t h, Vec4i tile) {
        auto sum = 0.f;
        auto count = 0u;
        for(auto j: range(tile.w)) {
            for(auto i: range(tile.z)) {
                auto x = tile.x + i;
                auto y = tile.y + j;
                if(x >= 0 && x < w && y >= 0 && y < h) {
                    sum += pData[x + y * w];
    ...
    float computeMeanLocalVariance(const Image& image, Vec4i tile, int windowSize) {
        auto sum = 0.f;
        auto count = 0u;
        for(auto j: range(tile.w)) {
            for(auto i: range(tile.z)) {
                if(image.contains(tile.x + i, tile.y + j)) {
                    sum += computeLocalVariance(image, Vec2u(tile.x + i, tile.y + j), windowSize);
                    ++count;
                }
            }
        }
