...
void ShowExampleAppCustomNodeGraph(ImVector<Node>& nodes, ImVector<NodeLink>& links, size_t width, size_t height, bool* opened)
{
...
public:
    ...
    DummyNode(int id, const char* name, const ImVec2& pos, float value, const ImVec4& color, int inputs_count, int outputs_count) {
...
void ComputeGraph::drawGUI(size_t width, size_t height, bool* pOpened) {
    ...
    if (!inited)
    {
        m_Nodes.emplace_back(std::make_unique<DummyNode>(0, "MainTex", ImVec2(40, 50), 0.5f, ImColor(255, 100, 100), 1, 1));
        m_Nodes.emplace_back(std::make_unique<DummyNode>(1, "BumpMap", ImVec2(40, 150), 0.42f, ImColor(200, 100, 200), 1, 1));
        m_Nodes.emplace_back(std::make_unique<DummyNode>(2, "Combine", ImVec2(270, 80), 1.0f, ImColor(0, 200, 100), 2, 2));
        //links.push_back(NodeLink(0, 0, 2, 0));
        //links.push_back(NodeLink(1, 0, 2, 1));
        inited = true;
    }
    ...
    {
        DummyNode* node = (DummyNode*) m_Nodes[node_idx].get();
        ImGui::PushID(node->ID);
        if (ImGui::Selectable(node->Name, node->ID == node_selected))
            node_selected = node->ID;
        if (ImGui::IsItemHovered())
    ...
    if (plug_selected_type > 0) {
        DummyNode& node = (DummyNode&)m_Nodes[plug_selected_node_idx];
        ImVec2 p1 = offset + (plug_selected_type == 1 ? node.GetOutputSlotPos(plug_selected) : node.GetInputSlotPos(plug_selected));
        ImVec2 p2 = ImGui::GetMousePos();
        draw_list->AddLine(p1, p2, ImColor(200, 200, 100), 3.0f);
    }
    ...
    if (ImGui::BeginPopup("context_menu"))
    {
        DummyNode* node = node_selected != -1 ? (DummyNode*) m_Nodes[node_selected].get() : NULL;
        ImVec2 scene_pos = ImGui::GetMousePosOnOpeningCurrentPopup() - offset;
        if (node)
        {
            ImGui::Text("Node '%s'", node->Name);
        ...
        {
            if (ImGui::MenuItem("Add")) { m_Nodes.emplace_back(std::make_unique<DummyNode>(m_Nodes.size(), "New node", scene_pos, 0.5f, ImColor(100, 100, 200), 2, 2)); }
            if (ImGui::MenuItem("Paste", NULL, false, false)) {}
        }
        ImGui::EndPopup();
    }
